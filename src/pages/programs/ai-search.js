...
export function initAISearch() {
  ...
  runBtn?.addEventListener("click", async () => {
    ...
    try {
      const ai = await callAIProvider(q, pruned);
...
/**
 * Call AI provider to search for programs matching a query.
 ...
 */
async function callAIProvider(query, pruned) {
  const systemPrompt = `You are a system administrator helping users find tools.
...
Return an empty results array if no programs match.`;
  ...
  const userPrompt = `Available programs (pruned):
