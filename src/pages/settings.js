...
async function renderRequired() {
  ...
  const rows = REQUIRED.map((req) => {
    ...
    const pathHtml = path
      ? `<div class="muted" title="${escapeHtml(path)}">${escapeHtml(
          path
        )}</div>`
      : `<div class="muted">${escapeHtml(req.hint || "")}</div>`;
    const btn = has
      ? ""
...
export async function initPage() {
  ...
  (async function () {
    ...
    async function load() {
      try {
        settings = await invoke("load_app_settings");
      } catch {
        settings = {};
      }
    ...
    function save() {
      return invoke("save_app_settings", { data: settings });
    }
    function escapeHtml(s) {
      return String(s).replace(
        /[&<>"']/g,
    ...
    function renderLinks() {
      ...
      if (!arr.length) {
        list.innerHTML = '<div class="muted">No links added.</div>';
        return;
      }
      list.innerHTML = arr
        .map(
          (l) =>
            `<div class="row" data-id="${
              l.id
            }"><div class="main"><div class="name">${escapeHtml(
              l.title || l.url
            )}</div><div class="muted" style="font-size:11px;">${escapeHtml(
              l.url
            )}</div></div><div class="meta" style="display:flex;gap:6px;"><button data-action="edit" class="ghost" title="Edit" style="min-width:42px;">Edit</button><button data-action="remove" class="danger" title="Remove" style="min-width:42px;">✕</button></div></div>`
        )
        .join("");
      list.querySelectorAll('button[data-action="remove"]').forEach((btn) => {
        btn.addEventListener("click", (e) => {
          ...
          settings.technician_links = settings.technician_links.filter(
            (x) => x.id !== id
          );
          save().then(() => {
