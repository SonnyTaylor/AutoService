...
function render(info) {
  ...
  if (info.motherboard) {
    ...
    section.insertAdjacentHTML(
      "beforeend",
      makeCollapsible("Motherboard", mbHtml)
    );
  }
  ...
  const avgCpu = cores.length
    ? Math.max(
        ...
        Math.min(
          100,
          Math.round(
            cores.reduce((s, c) => s + (c.usage_percent || 0), 0) / cores.length
          )
        )
      )
    : null;
    ...
    <div class="per-core-grid">
      ${cores
        .map((c) => {
          const pct = Math.max(
            0,
            Math.min(100, Math.round(c.usage_percent || 0))
          );
  ...
  if (ex && Array.isArray(ex.ram_modules) && ex.ram_modules.length) {
    ...
    const dimmRows = ex.ram_modules
      .map((m) => {
        ...
        const volt =
          m?.ConfiguredVoltage != null
            ? `${Number(m.ConfiguredVoltage) / 1000} V`
            : "-";
        const dtype = m?.MemoryType != null ? String(m.MemoryType) : "-";
        const ff = mapFF(Number(m?.FormFactor));
        const width =
          m?.DataWidth != null || m?.TotalWidth != null
            ? `${m?.DataWidth ?? "-"}/${m?.TotalWidth ?? "-"}`
            : "-";
        return `<tr>
  ...
  const ramHtml = `
    ...
    totalMem
  )} <span class="badge">${memPct}%</span><div class="progress" aria-label="memory usage"><div class="bar" style="width:${memPct}%;"></div></div></td></tr>
        ...
        <tbody>
          ${info.disks
            .map((d) => {
              ...
              const pct = d.total_space
                ? Math.min(100, Math.round((used / d.total_space) * 100))
                : 0;
              const flags = [
                `${d.is_removable ? "Removable" : ""}`,
                `${d.is_read_only ? "Read-only" : ""}`,
                `${d.kind ? escapeHtml(d.kind) : ""}`,
              ]
                .filter(Boolean)
                .join(", ");
              return `
              <tr>
                ...
                <td>R:${formatBytes(d.read_bytes)} • W:${formatBytes(
                d.written_bytes
              )}</td>
              </tr>`;
            })
  ...
  section.insertAdjacentHTML(
    "beforeend",
    makeCollapsible("Storage", storageHtml)
  );
        ...
        <tbody>
          ${info.networks
            ...
            <tr>
              ...
              <td>Rx ${formatBytes(n.total_received)}<br>Tx ${formatBytes(
                n.total_transmitted
              )}</td>
              <td>Rx ${formatBytes(n.received)} • Tx ${formatBytes(
                n.transmitted
              )}</td>
              <td>${
                n.errors_rx || n.errors_tx
                  ? `${n.errors_rx}/${n.errors_tx}`
                  : "-"
              }</td>
            </tr>
  ...
  // Battery (always render; supports multiple)
  {
      ...
      battHtml = batteries
        .map((batt, idx) => {
          ...
          const details = [
            ...
            batt.energy_full_wh != null
              ? `Full ${batt.energy_full_wh.toFixed(1)} Wh`
              : null,
            batt.energy_full_design_wh != null
              ? `Design ${batt.energy_full_design_wh.toFixed(1)} Wh`
              : null,
          ]
            .filter(Boolean)
            .join(" • ");
          ...
          <div class="table-block"><div class="table-wrap">
            <table class="table kv-table">
              <tbody>
                <tr><th>Charge ${
                  batteries.length > 1 ? `(Battery ${idx + 1})` : ""
                }</th><td><span class="badge ${stateBadgeClass}">${pct.toFixed(
            0
          )}%</span> <span class="muted" style="margin-left:8px;">${escapeHtml(
            batt.state || "-"
          )}</span></td></tr>
                ${
                  idBits
                ...
                ${
                  batt.time_to_full_sec != null
                    ? `<tr><th>To full</th><td>${formatDuration(
                        batt.time_to_full_sec
                      )}</td></tr>`
                    : ""
                }
                ${
                  batt.time_to_empty_sec != null
                    ? `<tr><th>To empty</th><td>${formatDuration(
                        batt.time_to_empty_sec
                      )}</td></tr>`
                    : ""
                }
              </tbody>
    ...
    section.insertAdjacentHTML(
      "beforeend",
      makeCollapsible("Battery", battHtml)
    );
  }
...
export async function initPage() {
  ...
  skel.innerHTML =
    '<div class="spinner" aria-hidden="true"></div><div><div class="loading-title">Loading system information…</div><div class="muted">Collecting hardware and OS details</div></div>';
  ...
  try {
    ...
    container.innerHTML =
      '<section class="page"><h1>System Info</h1><p class="muted">Failed to read system information.</p></section>';
    console.error(e);
  }
}
