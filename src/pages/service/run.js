...
const ATOMIC_TASKS = {
  adwcleaner_clean: {
    ...
    group: 'Cleanup',
    async build() {
      return { type: 'adwcleaner_clean', executable_path: await toolPath('adwcleaner'), working_path: '..\\data\\logs', clean_preinstalled: false };
    }
  },
  bleachbit_clean: {
    ...
    group: 'Cleanup',
    async build() {
      return { type: 'bleachbit_clean', executable_path: await toolPath('bleachbit'), options: ['system.tmp', 'system.recycle_bin', 'system.prefetch'] };
    }
  },
  dism_health_check: {
  ...
  smartctl_report: {
    ...
    group: 'Diagnostics',
    async build() { return { type: 'smartctl_report', executable_path: await toolPath('smartctl'), detail_level: 'basic' }; }
  },
  furmark_stress_test: {
...
export async function initPage() {
  ...
  backBtn?.addEventListener('click', () => { window.location.hash = '#/service'; });
  ...
  function renderParamControls(id, params) {
    ...
    wrapper.querySelectorAll('input').forEach(inp => {
      inp.addEventListener('change', () => {
        state[id].params[inp.dataset.param] = Number(inp.value) || state[id].params[inp.dataset.param];
        updateJson();
      });
    });
    return wrapper;
  ...
  function renderGpuSubOptions() {
    ...
    div.innerHTML = `
      <label><input type="checkbox" data-sub="furmark" ${gpuSubs.furmark ? 'checked' : ''}> FurMark <input type="number" class="dur" data-sub-dur="furmarkMinutes" value="${gpuParams.furmarkMinutes}" min="1" max="240" step="1"/></label>
      <label><input type="checkbox" data-sub="heavyload" ${gpuSubs.heavyload ? 'checked' : ''}> HeavyLoad <input type="number" class="dur" data-sub-dur="heavyloadMinutes" value="${gpuParams.heavyloadMinutes}" min="1" max="240" step="1"/></label>
    `;
    div.querySelectorAll('input[type="checkbox"]').forEach(cb => cb.addEventListener('change', () => {
      gpuSubs[cb.dataset.sub] = cb.checked;
      updateJson();
    }));
    div.querySelectorAll('input.dur').forEach(inp => inp.addEventListener('change', () => {
      gpuParams[inp.dataset.subDur] = Number(inp.value) || gpuParams[inp.dataset.subDur];
      updateJson();
    }));
    return div;
  }
  ...
  function renderItem(id) {
    ...
    checkbox.addEventListener('change', () => {
      if (checkbox.checked) { selection.add(id); if (!order.includes(id)) order.push(id); }
      else { selection.delete(id); order = order.filter(x => x !== id); }
      renderPalette();
    });
  ...
  function renderPalette() {
    paletteEl.innerHTML = '';
    order.forEach(id => { if (!['furmark_stress_test', 'heavyload_stress_gpu'].includes(id)) paletteEl.appendChild(renderItem(id)); });
    Object.keys(ATOMIC_TASKS).concat(GPU_PARENT_ID).forEach(id => { if (!['furmark_stress_test', 'heavyload_stress_gpu'].includes(id) && !selection.has(id)) paletteEl.appendChild(renderItem(id)); });
  ...
  async function generateTasksArray() {
    ...
    for (const id of order) {
      ...
      if (id === GPU_PARENT_ID) {
        if (gpuSubs.furmark) result.push(await ATOMIC_TASKS.furmark_stress_test.build({ params: { minutes: gpuParams.furmarkMinutes } }));
        if (gpuSubs.heavyload) result.push(await ATOMIC_TASKS.heavyload_stress_gpu.build({ params: { minutes: gpuParams.heavyloadMinutes } }));
        continue;
      }
    ...
    return result.filter(t => !('executable_path' in t) || !!t.executable_path);
  }
