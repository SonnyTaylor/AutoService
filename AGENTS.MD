...
**Location**: `src/utils/settings-manager.js`

AutoService uses a centralized, type-safe settings management system with:

- **Zod schema validation** - Automatic validation of all settings
- **Automatic caching** - 60-second cache with manual invalidation
- **Event-driven updates** - Subscribe to setting changes via pub-sub
- **Debounced writes** - 500ms debounce to batch rapid changes
...
- `business` - Business/technician mode settings
- `reports` - Auto-save, notifications, completion sounds
- `network_sharing` - Network path for shared report storage
- `network` - iperf server, ping host
- `ai` - OpenAI API key
- `sentry` - Error tracking configuration
- `sentry_enabled` - Master telemetry toggle

**Migration from Old Pattern**:

```javascript
// ❌ Old: Manual load/modify/save (duplicated everywhere)
const settings = await invoke('load_app_settings');
settings.business = settings.business || {};
settings.business.name = newName;
await invoke('save_app_settings', { data: settings });
clearBusinessCache();

// ✅ New: Centralized manager (60% less code)
await settingsManager.set('business.name', newName, true);
```
...
## AI Utilities - Multi-Provider Support

**Location**: `runner/ai_utils.py` | **Documentation**: `runner/AI_UTILITIES_README.md`

AutoService uses **LiteLLM** for unified AI operations across multiple providers while maintaining backward compatibility with OpenAI.
...
result = call_ai_analysis(
    ...
    model="gpt-4-turbo-preview",  # or "anthropic/claude-3-sonnet-20240229"
    api_key=api_key,
    temperature=0.3,
    json_mode=True,
    required_fields=["analysis", "recommendations"]  # Validates response
)
...
Services using AI utilities:
- `ai_startup_service.py` - Startup item optimization
- `ai_browser_notification_service.py` - Browser notification analysis
- `windows_update_logs_service.py` - Windows Update error analysis

### Model Name Format

Use `provider/model` format:
- `openai/gpt-4-turbo-preview` (or just `gpt-4-turbo-preview`)
...
- **Automatic JSON Parsing**: Extracts JSON from various response formats
- **Sentry Integration**: Automatic breadcrumb tracking
- **Fallback Support**: Falls back to requests library if LiteLLM unavailable
- **Backward Compatible**: Existing OpenAI code continues to work
...
**Release Format**: `autoservice@{version}` (e.g., `autoservice@0.5.0`)
